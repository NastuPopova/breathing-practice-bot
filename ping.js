 ...
 * @param {string} url - URL вашего приложения для пинга (должен быть абсолютным, например, https://example.com)
...
function setupPing(url, interval = 14) {
  ...
  const ping = async () => {
    try {
      logWithTime(`Выполняется пинг ${normalizedUrl}`);
      ...
      if (response.ok) {
        logWithTime(`Пинг успешен: ${normalizedUrl} (${response.status})`);
      ...
      if (failedAttempts >= MAX_FAILED_ATTEMPTS) {
        logWithTime(`КРИТИЧЕСКАЯ ОШИБКА: Не удается пропинговать ${normalizedUrl} ${failedAttempts} раз(а)`);

        // Отправляем аварийное уведомление в Telegram, используя глобальный объект бота
        try {
          const { bot, ADMIN_ID } = global.botData || {};
          if (bot && ADMIN_ID) {
            await bot.telegram.sendMessage(
              ADMIN_ID,
              `❗️ КРИТИЧЕСКАЯ ОШИБКА: Не удается пропинговать ${normalizedUrl} ${failedAttempts} раз(а)`
            );
          } else {
            logWithTime('Не удалось отправить уведомление: bot или ADMIN_ID не определены');
          }
        } catch (notificationError) {
          logWithTime(`Ошибка при отправке аварийного уведомления: ${notificationError.message}`);
        }
      }
    }
  };
  ...
  logWithTime(`Настроен пинг для ${normalizedUrl} с интервалом ${interval} минут`);
